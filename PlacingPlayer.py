...
def alphabeta_search(player, opponent, board, d=3):
    ...
    def max_value(current, alpha, beta, depth):
        ...
        for move in current.generate_placing_moves(player):
            value = max(value, min_value(current.place(player, move), alpha, beta, depth + 1))
            if value >= beta:
                return value
    ...
    def min_value(current, alpha, beta, depth):
        ...
        for move in current.generate_placing_moves(opponent):
            value = min(value, max_value(current.place(opponent, move), alpha, beta, depth + 1))
            if value <= alpha:
                return value
            beta = min(beta, value)
        return value
    ...
    action = max([(min_value(board.place(player, move), -sys.maxsize, sys.maxsize, 0), move) for move in
                  board.generate_placing_moves(player)],
